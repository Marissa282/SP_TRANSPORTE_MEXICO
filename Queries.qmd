...
- Propósito (P): 1 (Ir al hogar), 2 (Ir al trabajo), 3 (Ir a estudiar)... 10 (Otro).
...
```{python}
viajes=pd.read_csv("data/eod_2017_csv/tviaje_eod2017/conjunto_de_datos/tviaje.csv")
transportes= pd.read_csv("data/eod_2017_csv/ttransporte_eod2017/conjunto_de_datos/ttransporte.csv")
```
...
```{python}
df_finalisimo.to_csv("data/dag.csv", index=False, encoding="utf-8")
```
...
score(dag_1, data = data, type = "bic")
...
#BIC DAG 3
score(dag_3, data = data, type = "bic")


```

La dag 2 tiene el BIC mayor, por lo que se interpreta que es la red que mejor se ajusta a los datos. 
...
```{r}
best_dag = hc(data) #Hill Climbing con BIC 
```
...
# 7. Relaciones de nueva DAG

✏️ **Discutan la pertinencia de las relaciones de esta nueva DAG.**
...
✏️ **Respondan las queries asignadas.**
...
cpquery(bn, event = (O=="1" & P=="2" & D=="1"), evidence = (T=="2"|T=="5"|T=="6"|T=="8"|T=="10"|T=="11"|T=="12"|T=="13"|T=="15"), n = 10^6) 
```
## Query 2

¿Cuál es la probabilidad de que el estrato es bajo dado que su transporte sea un auto?

```{r}
cpquery(bn, event = (E=="2"), evidence = (T=="1"), n = 10^6) 
```
## Query 3
...
```{r}
cpquery(bn, event = (T=="2"|T=="5"|T=="8"|T=="6"), evidence = (E=="1"), n = 10^6) 
```
##  Query 4 
...
```{r}
cpquery(bn, event = S == "1" & E == "2", evidence = list(T = "16"), method = "lw")
```
